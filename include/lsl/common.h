...
/// Data format of a channel (each transmitted sample holds an array of
/// channels), 4 bytes wide
typedef enum {
  ...
  cft_int16 = 5,
  /// For binary signals or other coded data. Not recommended for encoding
  /// string data.
  cft_int8 = 6,
  /** 64 bit integers. Support for this type is not yet exposed in all
   * languages. Also, some builds of liblsl will not be able to send or receive
   * data of this type. */
  cft_int64 = 7,
...
_Static_assert(sizeof(lsl_channel_format_t) == 4,
               "lsl_channel_format_t size breaks the LSL ABI");
#elif defined(__cplusplus) && __cplusplus >= 201103L
static_assert(sizeof(lsl_channel_format_t) == 4,
              "lsl_channel_format_t size breaks the LSL ABI");
#elif !defined(LIBLSL_FFI)
static char
    _lsl_channel_format_size_check[1 -
                                   2 * !(sizeof(lsl_channel_format_t) == 4)];
#endif

/// Post-processing options for stream inlets.
typedef enum {
  /** No automatic post-processing; return the ground-truth time stamps for
     manual post-processing. This is the default behavior of the inlet. */
  proc_none = 0,
   ...
   *
   * This will apply a smoothing algorithm to the received time stamps; the
...
typedef enum {
  ...
  /// An argument was incorrectly specified (e.g., wrong format or wrong
  /// length).
...
typedef enum {
  /// Keep legacy behavior: max_buffered / max_buflen is in seconds; use asynch
  /// transfer.
  transp_default = 0,
 ...
 * acquired. If the "age" of a sample is known at a particular time (e.g., from
 * USB transmission delays), it can be used as an offset to lsl_local_clock() to
 * obtain a better estimate of when a sample was actually captured. See
 * lsl_push_sample() for a use case.
 */
